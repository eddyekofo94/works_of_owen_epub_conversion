...
- `express[ed` represents bracketed additions in the source text.
...
### Untranslated Latin
The following phrases appear as `Untranslated Latin Samples` but are intentionally skipped from bracketed translations because John Owen translated them inline immediately following the citation, or they are very short / proper nouns where an injected translation would break text flow:
* `Solis ad occasum, quum frigidus`
* `Sancto, † Athanas`
* `frustra praestantissimus sapientiae`
* `solitus est, si recludantur tyrannorum`
* `aspici laniatus et ictus; quando, ut corpora verberibus, ita saevitia`
* `malis consultis, animus dilaceretur`
* `et origo Trinitatis`
* `motus primo-primus`
* `opera naturae`
* `opera gratiae procuratae`
...
- The split near `" Malus bonum cum [Translated...` in Chapter 6 is triggered by injected translation brackets altering normal punctuation, but it remains structurally contiguous.
